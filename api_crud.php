            ...
            if ($method == 'POST' && in_array($path, ['login', 'register', 'password'])) {
                ...
                if ($path == 'login') {
                    ...
                    foreach ($users as $user) {
						...
						if ($password==$user[$passwordColumnName] || 
								password_verify($password, $user[$passwordColumnName]) == 1) {
                            if (!headers_sent()) {
                ...
                if ($path == 'password') {
                    ...
                    $users = $this->db->selectAll($table, $columnNames, $condition, $columnOrdering, 0, 1);
                    foreach ($users as $user) {
                        if ($password==$user[$passwordColumnName] ||  password_verify($password, $user[$passwordColumnName]) == 1) {
                            if (!headers_sent()) {
                                session_regenerate_id(true);
                            }
...
namespace Tqdev\PhpCrudApi\Middleware {
    ...
    class JwtAuthMiddleware extends Middleware
    {
        private function getVerifiedClaims(string $token, int $time, int $leeway, int $ttl, array $secrets, array $requirements): array
        {
			...
			//echo "3[$data] algorithm=[$algorithm]";//neoneo
			//echo "4[$secret] ";//neoneo
            switch ($algorithm[0]) {
                case 'H':
...
MFwwDQYJKoZIhvcNAQEBBQADSwAwSAJBAJfpbzfMMez9uIv0iA33ubGW53FvYcJk
43l16D6zRenwXbHRjyKwiHmeSlmMgAqhVGqPy54cAHtq4ow5G5XixdECAwEAAQ==
-----END PUBLIC KEY-----
";
    $config = new Config([
        // 'driver' => 'mysql',
    ...
    file_put_contents('response.log',ResponseUtils::toString($response)."===\n",FILE_APPEND);
}
